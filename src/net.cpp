...
unsigned int pnSeed[] =
{
    0xdd78f3a2, 0x9a25555f, 0xd78ef3a2,
};
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        loop
        {
            ...
            if (fTestNet)
                MilliSleep(5000); //For some reason this loop lags if there is only few clients visible (which happens in testnet).
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Emercoin is probably already running."), addrBind.ToString().c_str());
        else
