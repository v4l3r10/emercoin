    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %semc %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                           ...
                           list<pair<CTxDestination, int64> >& listSent, int64& nFee, string& strSentAccount, bool ignoreNameTx) const
{
    ...
    if (IsCoinStake())
    {
        ...
        {
            if (pwallet->IsMine(txout) && ExtractDestination(txout.scriptPubKey, address))
                break;
        }

        // nfee is negative for coinstake generation, because we are gaining money from it
        listReceived.push_back(make_pair(address, -nFee));
        nFee = 0;
        return;
    }
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        {
            // TODO: maybe add address exctraction from namecoin tx?
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %semc %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
	...
	unsigned int offset =   pbo->value.second;
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            ...
            if (CheckStakeKernelHash(nBits, block, offset, *pcoin.first, prevoutStake, txNew.nTime - n, hashProofOfStake))
            {
                ...
                if (block.GetBlockTime() + nStakeSplitAge > txNew.nTime && nCredit > nPoWReward)
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %semc %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %semc %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
