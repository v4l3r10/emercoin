                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    // gongxincoin: define some values used in case of namecoin tx creation
    ...
    txNew.nVersion = wtxNew.nVersion; // gongxincoin: important for name transactions

    {
        LOCK2(cs_main, cs_wallet);
        {
            nFeeRet = max(nFeeInput, MIN_TX_FEE);  // gongxincoin: a good starting point, probably...
            while (true)
            {
                txNew.vin.clear();
                txNew.vout.clear();
                ...
                // gongxincoin: in case of namecoin tx we have already supplied input.
		...
		// gongxincoin: name tx always at first position
                if (!wtxNameIn.IsNull())
                {
                ...
                BOOST_FOREACH(const PAIRTYPE(const CWalletTx*,unsigned int)& coin, setCoins)
                {
                    // gongxincoin: we sign name tx differently.
