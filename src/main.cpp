...
int nCoinbaseMaturity = COINBASE_MATURITY_EMC;
...
int64 CTransaction::GetMinFee2(unsigned int nBlockSize) const
{
                        ...
                        bool* pfMissingInputs)
{
    ...
    string strNonStd;
    if (!fTestNet && !tx.IsStandard(strNonStd) && !isNameTx)
    ...
    if (fCheckInputs)
    {
        ...
        if (!tx.AreInputsStandard(view) && !fTestNet && !isNameTx)
        ...
        int64 txMinFee = tx.GetMinFee2(1000);
        if (nFees < txMinFee)
            return error("CTxMemPool::accept() : not enough fees %s, %"PRI64d" < %"PRI64d,
                         hash.ToString().c_str(),
                         nFees, txMinFee);
        if (isNameTx && !hooks->IsNameFeeEnough(tx, nFees))
            return error("CTxMemPool::accept() : name fee is not enough for tx %s", hash.ToString().c_str());
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    if (GetDifficulty(nBits) < 512) {
        while (bnLowerBound + CENT <= bnUpperBound)
        {
            ...
            bnRewardPart = bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit * bnSubsidyLimit;
...
{
    static int64 nRewardCoinYear = 6 * CENT;  // creation amount per coin-year
...
bool CTransaction::CheckInputs(int64 &nTxFee, CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        if (pvChecks)
...
{
    int64 nTxFee = 0;
    return CheckInputs(nTxFee, state, inputs, fScriptChecks, flags, pvChecks);
}




bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean, bool fWriteNames)
{
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck, bool fWriteNames)
{
    ...
    for (unsigned int i=0; i<vtx.size(); i++)
    {
        ...
        else
        {
            ...
            if (!tx.CheckInputs(vFees[i], state, view, fScriptChecks, flags, nScriptCheckThreads ? &vChecks : NULL))
                return false;
            control.Add(vChecks);
        }
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    BOOST_FOREACH(CBlockIndex* pindex, vDisconnect) {
        ...
        int64 nStart = GetTimeMicros();
        if (!block.DisconnectBlock(state, pindex, view, NULL, true))
    ...
    BOOST_FOREACH(CBlockIndex *pindex, vConnect) {
        ...
        if (!block.ReadFromDisk(pindex))
            return state.Abort(_("Failed to read block"));
        int64 nStart = GetTimeMicros();
        if (!block.ConnectBlock(state, pindex, view, false, true)) {
            if (state.IsInvalid()) {
